...
if __name__ == '__main__':
    ...
    '''
    #show original testing data in 10 images
    raw_x_test_show = raw_x_test[:m2].reshape(-1,28,28)
    showimgs(raw_x_test_show, m, m, figsize=show_size)
    
    #show noising images
    x_test_show = x_test[:m2].reshape(-1,28,28)
    showimgs(x_test_show, m, m, figsize=show_size)
    
    #build model and training
    model = denoisingAE()
    model.compile(optimizer='sgd', loss='mse')
    model.fit(x_train.reshape(60000,28,28,1)/255., raw_x_train.reshape(60000,28,28,1)/255., batch_size=32, epochs=20)
    pre = model.predict(x_test.reshape(10000,28,28,1)/255.)
    pre*=255
    #show predict testing data in 8 images
    pre_show = pre[:m2].reshape(-1,28,28) 
    showimgs(pre_show, m, m, figsize=show_size)
